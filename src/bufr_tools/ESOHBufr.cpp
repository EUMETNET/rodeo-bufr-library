...
std::list<std::string> ESOHBufr::msg() const {
  ...
  for (auto s : desc) {
    ...
    for (std::list<Descriptor>::const_iterator ci = s.begin(); ci != s.end();
      ...
      {
        std::string value_str = getValue_s(v, false);
        ...
        case 1: // platform
        {
          ...
          {
            int bufr_state_id = getValue_i(v);
            lb.addLogEntry(
...
#pragma GCC diagnostic ignored "-Wmaybe-uninitialized"
          ...
          {
            int raw_time_disp = getValue_i(v);
              ...
              if (valid_period) {
                if ((data_category == 2 && int_data_subcategory == 1) ||
                  ...
                  if (start_end_period) {
                    ...
                    pi--;
                    if (*pi == *ci) {
                      int time_period_start = getValue_i(*pi);
...
bool ESOHBufr::addDescriptor(
  ...
  if (meta->unit() == "Numeric") {
    mvalue.SetUint64(std::stoi(getValue_s(v, false)));
  } else {
    std::string tmp_value = (getValue_s(v, false));
...
bool ESOHBufr::addContent(const Descriptor &v, std::string cf_name,
  ...
  std::string value_str =
      force_value.size() ? force_value : getValue_s(v, false);
  ...
  for (std::list<Descriptor>::const_iterator di = s.begin(); di != ci; ++di) {
    if (di->f() == 0 && di->x() == 1) {
      if (*di == DescriptorId("001101")) {
        int bufr_state_id = getValue_i(*di);
